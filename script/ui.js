...
var UI = {
    ...
    initSettings: function() {
        ...
        var websockifyPathRegex = /n\d{1,2}\//.exec(window.location.pathname);
        var websockifyPath = 'websockify';
        if (websockifyPathRegex && websockifyPathRegex.length) {
            websockifyPath = websockifyPathRegex[0] + websockifyPath;
        }
        window.localStorage.setItem("path", websockifyPath);
        console.log("[MISA]websockify path=", websockifyPath);
        UI.initSetting('path', websockifyPath);
